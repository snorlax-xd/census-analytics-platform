...
### 2.1 Metric selector
- A control above the map (button group or select, not a dropdown reused from elsewhere) lets the user choose which metric colors the map: population, population density, sex ratio, literacy rate (list extensible later). **Note: `population_density` requires a supplementary state land-area figure not present in the primary dataset (`india-districts-census-2011.csv` has no area/km² column) — see `DATABASE.md` §3/§8.**
- Changing the metric:
...
- Finalize the exact v1 metric list beyond population. Sex ratio and literacy rate are confirmed (both derivable directly from the primary CSV). **Population density is not yet confirmed** — it needs a supplementary state land-area figure the primary dataset doesn't include (see `DATABASE.md` §3/§8); decide whether to source that before launch or defer the metric.
