...
- The reference design's sidebar had inconsistent nested categories (e.g. "Literacy" containing sub-items also called "Demographics" and "Literacy"). This is corrected to a **single flat level**: Demographics, Economy, Literacy, Infrastructure, Sanitation, Drinking Water — no nested nav nesting. `Health` was dropped (no supporting data in the primary source, `india-districts-census-2011.csv`); Sanitation and Drinking Water are split out as their own dedicated categories instead of folded into Infrastructure, since the source data is granular enough on both to warrant it (see `DATABASE.md` §8). Selecting a category filters which chart panels are shown below; no multi-level tree to build or maintain.
...
### 6.4 Radar chart
- Multi-metric composite view of normalized indicators (e.g. literacy, sex ratio, drinking water access, infrastructure) across State A vs State B — clean at exactly two states, since we're not fighting radar-chart overlap issues.
...
- **Indexing note:** the "closest value" preset query benefits from an index on `(measure_id, year, value)` in the fact table, since it's effectively a nearest-neighbor sort on `value` filtered by measure and year — worth adding if this query is used frequently.
...
- ~~Confirm the full v1 metric list per category~~ — resolved: `demographics`, `economy`, `literacy`, `infrastructure`, `sanitation`, `drinking_water`. Full column-to-category mapping is in `DATABASE.md` §8.
